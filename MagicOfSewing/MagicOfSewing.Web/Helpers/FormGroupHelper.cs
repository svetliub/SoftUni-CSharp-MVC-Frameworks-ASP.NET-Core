namespace MagicOfSewing.Web.Helpers
{
    ...
    public static class FormGroupHelper
    {
        public static IHtmlContent FormGroupFor<TModel, TResult>(this IHtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TResult>> expression)
        {
            ...
            {
                var label = htmlHelper.LabelFor(expression, new { @class = "form-group" });
                var editor = htmlHelper.EditorFor(expression, new { htmlAttributes = new { @class = "form-control", @placeholder = "..." } });
